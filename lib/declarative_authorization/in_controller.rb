...
module Authorization
  module AuthorizationInController
    ...
    def load_controller_object (context_without_namespace = nil) # :nodoc:
      ...
      instance_variable_set(instance_var, orm_interface.find(model, params[:id]))
    end

    def orm_interface
      @orm_interface ||= Authorization::Orms::Interface.new
    end
    ...
    def load_parent_controller_object (parent_context_without_namespace) # :nodoc:
      ...
      instance_variable_set(instance_var, orm_interface.find(model, params[:"#{parent_context_without_namespace.to_s.singularize}_id"]))
    end
  ...
  class ControllerPermission # :nodoc:
    ...
    def load_object(contr)
      if @load_object_method and @load_object_method.is_a?(Symbol)
        ...
        unless object
          begin
            object = orm_interface.find(load_object_model, contr.params[:id])
